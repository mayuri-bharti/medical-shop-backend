...
router.post('/facebook', [
  ...
  try {
    ...
    if (!process.env.FACEBOOK_APP_ID || !process.env.FACEBOOK_APP_SECRET) {
      ...
      return res.status(500).json({
        success: false,
        message: 'Facebook OAuth not configured. Please contact administrator.'
      })
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let errorMessage = 'Invalid Facebook token. Please try again.'
        if (errorData.error?.message?.includes('expired')) {
          errorMessage = 'Token expired. Please sign in with Facebook again.'
        } else if (errorData.error?.message?.includes('invalid')) {
          errorMessage = 'Invalid token. Please check Facebook OAuth configuration.'
        }
        
        return res.status(400).json({
          success: false,
          message: errorMessage,
          shouldRetry: errorData.error?.message?.includes('expired'),
          ...(process.env.NODE_ENV === 'development' && { 
      ...
      try {
        ...
        if (debugResponse.ok) {
          ...
          if (!debugData.data?.is_valid || debugData.data?.app_id !== process.env.FACEBOOK_APP_ID) {
            return res.status(400).json({
              success: false,
              message: 'Invalid token. Token does not belong to this application.'
            })
          }
        }
      } catch (debugError) {
        // If debug token fails, log but continue (token might still be valid)
        console.warn('Facebook debug token check failed:', debugError.message)
      }
      ...
      console.error('❌ Facebook token verification error:', {
        message: facebookError.message,
        name: facebookError.name
      })
      
      return res.status(400).json({
        success: false,
        message: 'Failed to verify Facebook token. Please try again.',
        ...(process.env.NODE_ENV === 'development' && { 
          details: facebookError.message
        })
      })
    }
    ...
    if (!facebookId || !email) {
      return res.status(400).json({
        success: false,
        message: 'Facebook account information incomplete. Email is required.'
      })
    }
